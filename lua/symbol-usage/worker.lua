...
local git = require('symbol-usage.git')
...
function W:collect_symbols(symbol_tree)
  ...
  local function _walk(symbols, parent, booked_lines)
    ...
    for _, symbol in ipairs(sorted_symbol_tree) do
      ...
      if pos and #allowed_methods > 0 then
        ...
        if self.opts.git.enabled and git.is_cache_valid(self.bufnr) then
          local info = git.get_line_info(self.bufnr, pos.line)
          if info then
  ...
  if self.opts.git.enabled and not git.is_cache_valid(self.bufnr) then
    git.fetch_async(self.bufnr, function(ok)
      if ok then
        self:refresh_git_extmarks()
      end
    end)
  end
...
function W:set_extmark(symbol_id, line, count, id)
  ...
  if self.symbols[symbol_id] then
    local data = self.symbols[symbol_id]
    if count then
...
---Refresh extmarks with git info after async fetch
function W:refresh_git_extmarks()
  for symbol_id, symbol in pairs(self.symbols) do
    if not symbol.is_stacked and symbol.mark_id then
      local pos = u.get_position(symbol.raw_symbol, self.opts)
      if pos then
        local info = git.get_line_info(self.bufnr, pos.line)
        if info then
